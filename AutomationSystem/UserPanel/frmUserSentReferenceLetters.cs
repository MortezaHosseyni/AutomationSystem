...
namespace AutomationSystem.UserPanel
{
    public partial class frmUserSentReferenceLetters : Form
    {
        ...
        private void frmUserSentReferenceLetters_Load(object sender, EventArgs e)
        {
            ...
            ShowSentReferenceLetters(searchCondition());
        }

        private void ShowSentReferenceLetters(string searchReference)
        {
            var query = db.Database.SqlQuery<Vw_ReciveReference>($"SELECT * FROM Vw_ReciveReference WHERE LetterDraftType = 2 AND RefSenderUserID = {PublicVariable.global_UserID} {searchReference}");
            ...
            if (result.Count != 0)
            {
                ...
                for (int i = 0; i <= result.Count - 1; i++)
                {
                    ...
                    dgv_RecivedReferenceLetters.Rows[i].Cells["col_ReferenceReciver"].Value = result[i].RefReciver_FullName;
                    dgv_RecivedReferenceLetters.Rows[i].Cells["col_RefCaption"].Value = result[i].RefCaption;
                    dgv_RecivedReferenceLetters.Rows[i].Cells["col_ReferenceReciverUserID"].Value = result[i].RefReciverUserID;
        ...
        private string searchCondition()
        {
            ...
            {
                searchString += $" AND RefReciver_FullName LIKE '%{txt_LetterReciver.Text}%'";
            }
